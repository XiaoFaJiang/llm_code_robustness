...
<<<<<<< HEAD
    def init_pretrained_model(self,path="microsoft/codebert-base-mlm"):
=======
    def init_pretrained_model(self,path="/data1/ljc/code/llm_robustness_eval_and_enhance/model/codebert/codebert-mlm"):
>>>>>>> 1e84c061da6913b393aba496a49d441adacc38c3
        '''
    ...
    def __get_function_names(self,code:str) -> List[str]:
        '''
        获得一段代码中所有的函数名,及其对应于tree的节点node
        '''
        tree = self.__parser.parse(bytes(code, 'utf-8'))
    ...
    def random_filp_function_name(self,code:str) -> str:
        _,function_names = self.__get_function_names(code)
        ...
        code = self.__rename_function_name(code,func_name,substitue)
    ...
    def normalize_function_names(self,code:str) -> str:
        ...
        for i,fcn in enumerate(function_names):
            ret_code = self.__rename_function_name(ret_code,fcn,"func_" + str(i))[:]
            ...
            if words:
                for oneword in words:
                    ...
                    if  now in exist_words:
                        continue
                    ret_code = self.__rename_function_name(ret_code,fcn,now)[:]
                    break
        return self.__post_preprocess_code(ret_code)
    ...
    def codebert_rename_func_name(self,code:str) -> str:
        ...
        def rename_once(renamed_name,ret_code):
            ...
            input_pred = self.__rename_function_name(ret_code,renamed_name,"<mask>")
            ...
            if "<mask>" in input_pred:
                for token_info in self.pipeline(input_pred):
                    ...
                    if name and self.__is_valid_identifier_name(name,self.lang) and name not in exist_words:
                        ret_code = self.__rename_function_name(ret_code,renamed_name,name)
