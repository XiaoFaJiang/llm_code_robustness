...
if __name__ == '__main__':
    ...
    print(f"calculate model:{args.model_name} perturbation:{args.perturbation} language:{args.language}")
    ...
    if args.model_type == "causal_base":
        perturbation_whole_name = f"mbpp_generate_{args.language}_robust_{args.perturbation}{args.prompt_type}"
        original_cases = json.load(open(os.path.join(prefix_path,args.model_name,"evaluation_results",f"mbpp_generate_{args.language}_robust_no_change{args.prompt_type}","evaluation_cases.json"),"r"))
        
    else:
        perturbation_whole_name = f"mbpp_generate_{args.language}_robust_{args.perturbation}_instruct{args.prompt_type}"
        original_cases = json.load(open(os.path.join(prefix_path,args.model_name,"evaluation_results",f"mbpp_generate_{args.language}_robust_no_change_instruct{args.prompt_type}","evaluation_cases.json"),"r"))
