...
class GeneralPerturbedHumanEval(Task):
    ...
    DATASET_PATH= "/mnt/dolphinfs/hdd_pool/docker/user/hadoop-aipnlp/llm-eval/benchmark/bigcode/perturbed_humaneval"
    ...
    def process_results(self, generations, references):
        ...
        metrics = [rp1[perturbation_name]["robust-pass-at-1"] for perturbation_name in rp1.keys()]
        stat = {}
        stat["name"] = {"name": "quasi_prefix_exact_match", "split": "test"}
        stat["count"] = len(references)
        sum_ = np.mean(metrics) * stat["count"]
        stat["sum"] = sum_
        stat["sum_squared"] = sum_ * sum_
        stat["min"] = np.mean(metrics)
        stat["max"] = np.mean(metrics)
        stat["mean"] = np.mean(metrics)
        stat["variance"] = 0.0
        stat["stddev"] = 0.0
        stats = [[stat, f"recode_zero_shot:perturbed_humaneval_{self.category}_num_seeds_{self.num_seeds}-generation-generation,"]]

        return rp1, None, stats
